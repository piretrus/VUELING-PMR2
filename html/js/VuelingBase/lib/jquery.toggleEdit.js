/*! jQuery Toggle Edit | v1.6 | jqueryui.com */
(function ($) {
    ...
    $.widget("ui.toggleEdit", {
        ...
        _init: function () {
            ...
            self.element.addClass("toggleEdit toggleEdit-edit toggleEdit-edit-" + self._tag(self.element)).data("toggleEdit-preview", self.p);
            self.eventsEnabled = self.options.eventsEnabled;

            /* VUELING CUSTOM */
            if (self.element.is("input[type='radio']")) {
                var id = $(self.element).attr('id'),
                    checked = $(self.element).attr('checked')
                label = $("label[for='" + id + "']");
                if (!checked)
                    $(label).hide();
            }
        ...
        _getTypeInput: function (e) {
            var self = this;
            var i;
            for (i = 0; i < self.options.types.length; i++) {
