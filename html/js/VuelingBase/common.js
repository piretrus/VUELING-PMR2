...
SKYSALES.Class.Resource = function ()
{
    ...
    */
    thisResource.populateMarketHash = function ()
    {
        ...
        var destination = {};
        var station = {};
        var stationHash = thisResource.stationHash;
        ...
        if (thisResource.marketInfo && thisResource.marketInfo.MarketList)
        {
            ...
            for (i = 0; i < marketArray.length; i += 1)
            {
                ...
                if (destinationArray)
                {
                    ...
                    destinationIndex = 0;
                    while (destinationIndex < destinationArray.length)
                    {
                        destination = destinationArray[destinationIndex];
                        destinationCode = destination.code;
                        destination.name = '';
                        station = stationHash[destinationCode];
                        if (station)
                        {
                        ...
                        {
                            destinationArray.splice(destinationIndex, 1);
                            destinationIndex -= 1;
                        }
                        destinationIndex += 1;
                    }
...
SKYSALES.Util.parseIsoDate = function (dateStr)
{
    ...
    var matches = iso8601Regex.exec(dateStr);
    if (matches)
    {
...
if (!SKYSALES.Class.TravelDocumentInput)
{
    SKYSALES.Class.TravelDocumentInput = function () 
    {
        ...
        thisTravelDocumentInput.setTravelDocumentInfo = function ()
        {
            ...
            var birthCountry = this.documentBirthCountry.val();
...
SKYSALES.Class.ToggleView = function ()
{
    ...
    thisToggleView.addEvents = function ()
    {
        ...
        {
            this.show.toggle(this.updateShowHandler, this.updateHideHandler);
        }
        else
        {
...
if (!SKYSALES.Class.PaymentInput)
{
    SKYSALES.Class.PaymentInput = function ()
    {
        ...
        thisPaymentInput.setVarsAfterAjaxResponse = function (jData)
        {
            ...
            thisPaymentInput.foreignCurrencyName = this.getById(this.foreignCurrencyNameId, offerInfo).text();
...
if (!SKYSALES.Class.DatePickerManager)
{
    SKYSALES.Class.DatePickerManager = function ()
    {
        ...
        var yearMonthFormatString = 'yy-mm'; // remember that in these format strings, yy is a 4-digit year
        ...
        var readLinked = function ()
        {
            ...
            thisDatePickerManager.linkedDate.val(SKYSALES.Util.dateToIsoString(date));
            return {};
        };
        ...
        var dateUpdate = function ()
        {
            ...
            var dateVal = SKYSALES.Util.dateToIsoString(date);
        ...
        {
            var date = SKYSALES.Util.parseIsoDate(dateString);
            if (date)
            {
                var yearMonthString = $.datepicker.formatDate(yearMonthFormatString, date);
        ...
        thisDatePickerManager.addEvents = function ()
        {
            ...
            var datePickerParams =
            {
                ...
                'buttonImage': 'images/VuelingBase/calendar.png',
                'buttonText': 'Calendar',
                'numberOfMonths': 2,
                'mandatory': true,
...
SKYSALES.Class.CalendarAvailabilityMarket = function ()
{
    ...
    thisCalendarAvailabilityMarket.noFlightsTemplateId = 'noFlightsTemplateId';
    thisCalendarAvailabilityMarket.noFlightsTemplate = null;
    ...
    thisCalendarAvailabilityMarket.setVars = function ()
    {
        ...
        thisCalendarAvailabilityMarket.noFlightsTemplate = this.getById(this.noFlightsTemplateId);
    };
